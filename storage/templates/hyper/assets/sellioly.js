...
class Cart extends HTMLElement {
  ...
  getSectionsToRender() {
      return ['cart-notification-sidebar', 'cart-notification-icon'];
  }
...
class AddToCartForm extends Cart {
  ...
  handleVariantOptionRadioChange() {
      ...
      const variantOptions = this.querySelectorAll('variant-options-radio');
      variantOptions.forEach(optionInputs => {
          this.inputs = optionInputs.querySelectorAll('input[type=radio]');
          this.inputs.forEach((input) => {
              if (input.checked) {
  ...
  handleVariantOptionSelectChange() {
      ...
      const variantOptions = this.querySelectorAll('variant-options-select');
      variantOptions.forEach(selectInputs => {
...
class UpdateCartQuantityForm extends Cart {
  constructor() {
      ...
      this.form.querySelector('input[name="quantity"]').addEventListener('input', this.onSubmitHandler.bind(this));

      console.log(this.form.querySelector('input[name="quantity"]'));
  }

  onSubmitHandler(evt) {
      evt.preventDefault();

      console.log(this.form.quantity.value)

      if (this.form.quantity.value <= 0) {
          ...
          .then((res) => {
              ...
              this.querySelector('#order-now-form').innerHTML = `
                <div class=" text-center" style="padding-top: 3rem;border-top: 0.1rem solid #ebebeb;">
                    ...
                    <h4 class="text-center mb-10">Order completed!</h4>
                    <p class="icon-box-title">Order id: <strong>#${res.order_id}</strong></p>
                </div>
              `
