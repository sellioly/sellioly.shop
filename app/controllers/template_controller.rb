...
class TemplateController < ApplicationController
  ...
  def request_template_page
    ...
    begin
      ...
      raise TemplateFiles::TemplateFileService::FileNotFoundError, "Template file not found: #{e.message}"
    rescue JSON::ParserError => e
      raise TemplateFiles::TemplateFileService::InvalidFileTypeError, "Invalid JSON: #{e.message}"
    end
  end
  ...
  def request_asset_content
    ...
    raise TemplateFiles::TemplateFileService::FileNotFoundError, "File not found: #{key}" unless File.exist?(file_path)
    raise TemplateFiles::TemplateFileService::SecurityError, "Path outside template directory" unless file_path.start_with?(base_path)
  ...
  def update_asset_content
    ...
    raise TemplateFiles::TemplateFileService::FileNotFoundError, "File not found: #{key}" unless File.exist?(file_path)
    raise TemplateFiles::TemplateFileService::SecurityError, "Path outside template directory" unless file_path.start_with?(base_path)
    raise TemplateFiles::TemplateFileService::FileTooLargeError, "File too large" if content.bytesize > 10 * 1024 * 1024
  ...
  def request_template
    ...
    raise TemplateFiles::TemplateFileService::FileNotFoundError, "Template not found" unless Dir.exist?(base_path)
  ...
  def create_asset_directory
    ...
    raise TemplateFiles::TemplateFileService::SecurityError, "Path outside template directory" unless dir_path.start_with?(base_path)
  ...
  def move_asset_file
    ...
    raise TemplateFiles::TemplateFileService::FileNotFoundError, "File not found: #{old_key}" unless File.exist?(old_path)
    raise TemplateFiles::TemplateFileService::SecurityError, "Path outside template directory" unless old_path.start_with?(base_path) && new_path.start_with?(base_path)
  ...
  def upload_asset_file
    ...
    raise TemplateFiles::TemplateFileService::SecurityError, "Path outside template directory" unless upload_path.start_with?(base_path)
    raise TemplateFiles::TemplateFileService::FileTooLargeError, "File too large" if file.size > 10 * 1024 * 1024
      ...
      if File.exist?(full_path)
        ...
        if File.exist?(dir_path)
          ...
          unless real_dir.start_with?(real_base)
            raise TemplateFiles::TemplateFileService::SecurityError, "Path traversal detected"
          end
        end
  ...
  def validate_file_extension(filename)
    ...
    unless ALLOWED_EXTENSIONS.include?(ext)
      raise TemplateFiles::TemplateFileService::InvalidFileTypeError, "File type not allowed: #{ext}. Allowed types: #{ALLOWED_EXTENSIONS.join(', ')}"
    end
  end
  ...
  def validate_json_content(content, filename)
    ...
    begin
      ...
      raise TemplateFiles::TemplateFileService::InvalidFileTypeError, "Invalid JSON syntax in #{filename}: #{e.message}"
    end
  end
end
